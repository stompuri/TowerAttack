...
namespace TA
{
	CharacterFast::CharacterFast()
	{
		...
		rotation = 0.0f;
		moveClock.restart();
		Load("resources/images/texture_characterFast.png");
		assert(IsLoaded());
		...
		std::cout << "DEBUG <CharacterFast::Character()>: Character initialized\n";
	}

		
	CharacterFast::CharacterFast(unsigned int _placement)
	{
		ready = false;
		travelledTiles = 0;
		active = false;
		placement = _placement;
		structure = Game::GetBaseCFast()->GetStructure();
		armor = Game::GetBaseCFast()->GetArmor();
		shield.baseValue = Game::GetBaseCFast()->GetShield();
		shield.currentValue = shield.baseValue;
		speed = Game::GetBaseCFast()->GetSpeed();
		cost = 2;
		arrived = true;
		gameTurn = Game::GetPlayTurn();
		activePhase = "attack";
	...
	CharacterFast::CharacterFast(std::istream & is):Character(is) // Read data for the parent
	{
		std::cout << "DEBUG <CharacterFast::Character(is)>: ***** Loading data! *****\n";
		is.read((char*)&rotation, sizeof(float)); // Read rotation
		GetSprite().setRotation(rotation);
		std::cout << "DEBUG <CharacterFast::Character(is)>: Reading rotation: " + std::to_string(rotation) + "\n";
		is.read((char*)&maxVelocity, sizeof(float)); // Read maxVelocity
		std::cout << "DEBUG <CharacterFast::Character(is)>: Reading maxVelocity: " + std::to_string(maxVelocity) + "\n";
		// sf::Vector2f is handled in two parts
		float velX, velY;
		is.read((char*)&velX, sizeof(float)); // Read velocity.x
		std::cout << "DEBUG <CharacterFast::Character(is)>: Reading velocity.x: " + std::to_string(velX) + "\n";
		is.read((char*)&velY, sizeof(float)); // Read velocity.Y
		std::cout << "DEBUG <CharacterFast::Character(is)>: Reading velocity.y: " + std::to_string(velY) + "\n";
		velocity.x = velX;
		velocity.y = velY;

		is.read((char*)&targetPosition.x, sizeof(float)); // Read targetPosition.x
		std::cout << "DEBUG <CharacterFast::Character(is)>: Reading targetPosition.x: " + std::to_string(targetPosition.x) + "\n";
		is.read((char*)&targetPosition.y, sizeof(float)); // Read targetPosition.y
		std::cout << "DEBUG <CharacterFast::Character(is)>: Reading targetPosition.y: " + std::to_string(targetPosition.y) + "\n";
	
		lastDirection = SaveGameManager::ReadString(is);
		std::cout << "DEBUG <CharacterFast::Character(is)>: Read last direction: " + lastDirection + "\n";

		// Set origin, so the rotation works like it should...
		GetSprite().setOrigin(GetSprite().getGlobalBounds().height / 2, GetSprite().getGlobalBounds().width / 2);
		std::cout << "DEBUG <CharacterFast::Character(is)>: ***** Data loaded! *****\n";
	}

	void CharacterFast::serialise(std::ostream & os) // Write the data into the file
	{
		std::cout << "DEBUG <CharacterFast::serialise>: ***Saving Character!\n";
		Character::serialise(os);

		os.write((char*)&rotation, sizeof(float)); // Save rotation
		std::cout << "DEBUG <CharacterFast::serialise>: Saved rotation: " + std::to_string(rotation) + "\n";
		os.write((char*)&maxVelocity, sizeof(float)); // Save maxVelocity
		std::cout << "DEBUG <CharacterFast::serialise>: Saved maxVelocity: " + std::to_string(maxVelocity) + "\n";
		// sf::Vector2f is handled in two parts
		float velX = velocity.x;
		float velY = velocity.y;
		os.write((char*)&velX, sizeof(float)); // Save velocity.x
		std::cout << "DEBUG <CharacterFast::serialise>: Saved velocity.x: " + std::to_string(velocity.x) + "\n";
		os.write((char*)&velY, sizeof(float)); // Save velocity.y
		std::cout << "DEBUG <CharacterFast::serialise>: Saved velocity.y: " + std::to_string(velocity.y) + "\n";

		os.write((char*)&targetPosition.x, sizeof(float)); // Write targetPosition.x
		std::cout << "DEBUG <CharacterFast::serialise>: Saved targetPosition.x: " + std::to_string(targetPosition.x) + "\n";
		os.write((char*)&targetPosition.y, sizeof(float)); // Write targetPosition.y
		std::cout << "DEBUG <CharacterFast::serialise>: Saved targetPosition.y: " + std::to_string(targetPosition.y) + "\n";

		SaveGameManager::SerialiseString(os, lastDirection); // Write lastDirection
		std::cout << "DEBUG <CharacterFast::serialise>: Saved last direction: " + lastDirection + "\n";

		std::cout << "DEBUG <CharacterFast::serialise>: ***Data saved!\n";
	}

	/*void CharacterFast::Update(float elapsedTime)
	{
		if(!active) 
		{ // && placement == 0 && ) {
				...
				if (!arrived)
				{
						...
						velocity.y < 0 && targetPosition.y > _location.y) // we have moved over the target and moving away
					{
						...
						if(speed == travelledTiles)
						{
							...
							std::cout << "DEBUG<CharacterFast>: setting the next target\n";
					...
					std::cout << "DEBUG<CharacterFast>: Start the journey (towards the next tile)!\n";
	...
	std::string CharacterFast::GetType()
	{
		return "CharacterFast";
	}
}